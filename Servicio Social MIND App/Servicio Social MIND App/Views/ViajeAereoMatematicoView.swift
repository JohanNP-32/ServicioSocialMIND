...
// MARK: - 3. VISTA PRINCIPAL (MENÚ DEL GLOBO)
struct ViajeAereoMatematicoView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // --- CORRECCIÓN AQUÍ ---
            // Esto evita que los botones y el header "bailen" o se muevan al aparecer la pantalla.
...
// MARK: - 4. PANTALLA DE JUEGO (GAMEPLAY)
struct GamePlayView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                // --- HEADER MEJORADO (CENTRADO PERFECTO) ---
                ZStack {
                    // 1. Capa del Centro (Título del Nivel)
                    ...
                    // 2. Capa de los extremos (Botones)
                    HStack {
                        Button(action: { dismiss() }) {
                            Image(systemName: "arrow.left.circle.fill")
                                .font(.system(size: 45))
